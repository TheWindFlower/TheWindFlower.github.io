(() => {
    ...
    const gifLayer = document.getElementById("mxrn-gif-layer");

    const images = [
        { src: "/public/assets/img/ascii.png", alt: "" },
        { src: "/public/assets/img/ascii2.png", alt: "" },
        { src: "/public/assets/img/ascii.gif", alt: "" },
    ];
    ...
    function startGifPlayback(imgObj) {
        gifStopFn?.();

        gifPlaying = true;

        gifLayer.src = imgObj.src;
        ...
        function animate(currentTime) {
            ...
            for (let col = 0; col < cols; col++) {
                ...
                for (let i = 0; i < column.trailLength; i++) {
                    ...
                    const brightness = i === 0 ? 1 : Math.max(0, 1 - i / column.trailLength);

                    ctx.fillStyle =
                        i === 0
                            ? `rgba(200,255,200,${brightness})`
                            : `rgba(0,255,70,${brightness})`;

                    ctx.fillText(column.chars[row], x, y);
                }

                const revealRow = headRow - column.trailLength - 3;
